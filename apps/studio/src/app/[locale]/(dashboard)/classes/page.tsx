...
import { Plus, Edit, Trash2, Swords, AlertCircle, Search, X, ChevronLeft, ChevronRight } from 'lucide-react'
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-start justify-between">
                ...
                <Link href={`/${locale}/classes/create`}>
                    <Button size="sm" className="gap-1.5"><Plus className="h-4 w-4" />{t('newClass')}</Button>
                </Link>
            </div>
                ...
                <CardContent className="p-0">
                    {isLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-16 gap-3 text-muted-foreground">
                            ...
                            <p className="text-sm">{searchTerm ? tc('noResults') : t('noClasses')}</p>
                        </div>
                    ) : (
                        <Table>
                            <TableHeader>
                            ...
                            <TableBody>
                                {classList.map(c => (
                                    <TableRow key={c.id} className="group">
                                        ...
                                        <TableCell>
                                            <Link href={`/${locale}/classes/${c.id}`} className="font-medium hover:underline">{c.name}</Link>
                                        </TableCell>
                                        <TableCell className="font-mono text-xs text-muted-foreground">{c.slug ?? '—'}</TableCell>
                                        <TableCell className="text-sm text-muted-foreground truncate max-w-xs">{c.description ?? '—'}</TableCell>
                                        <TableCell className="text-right">
                                            <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                                                    <Link href={`/${locale}/classes/${c.id}`}><Edit className="h-4 w-4" /></Link>
                                                </Button>
                    ...
                    {/* Pagination */}
                    {!isLoading && classList.length > 0 && pag && (
            ...
            <Dialog open={!!deleteTarget} onOpenChange={open => { if (!open) setDeleteTarget(null) }}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        ...
                        <Button variant="destructive" disabled={deleteClass.isPending}
                            onClick={() => deleteTarget && deleteClass.mutate({ id: deleteTarget.id })}>
                            {deleteClass.isPending ? '…' : tc('delete')}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
