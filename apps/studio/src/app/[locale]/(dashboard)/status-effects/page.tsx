...
export default function StatusEffectsPage() {
    ...
    const [itemToDelete, setItemToDelete] = useState<StatusEffect | null>(null)
    ...
    const { data, isLoading, error, refetch } = trpc.statusEffects.list.useQuery({ search: searchTerm || undefined, page, pageSize: 20 })
    const deleteItem = trpc.statusEffects.delete.useMutation({ onSuccess: () => { refetch(); setItemToDelete(null) } })
    const list: StatusEffect[] = data?.data ?? []
        ...
        <TooltipProvider delayDuration={300}>
            <div className="space-y-6">
                <div className="flex items-start justify-between">
                    ...
                    <Link href={`/${locale}/status-effects/create`}>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent">
                                    ...
                                    <TableHead>{t('maxStacks')}</TableHead>
                                    <TableHead>{t('durationMs')}</TableHead>
                                    <TableHead>{t('isDebuff')}</TableHead>
                                    <TableHead>{t('isPurgeable')}</TableHead>
                            ...
                            <TableBody>
                                {list.length === 0 ? (
                                    ...
                                    <TableRow key={item.id} className="group cursor-pointer" onClick={() => router.push(`/${locale}/status-effects/${item.id}`)}>
                                        ...
                                        <TableCell className="text-sm">{item.maxStacks ?? '—'}</TableCell>
                                        <TableCell className="text-sm">{item.durationMs ?? '—'}</TableCell>
                                        <TableCell>
                                            <Badge variant={item.isDebuff ? 'destructive' : 'secondary'} className="text-xs">
                                                {item.isDebuff ? commonT('yes') : commonT('no')}
                                            </Badge>
                                        </TableCell>
                                        <TableCell>
                                            <Badge variant={item.isPurgeable ? 'default' : 'secondary'} className="text-xs">
                                                {item.isPurgeable ? commonT('yes') : commonT('no')}
                                            </Badge>
                                        </TableCell>
                                            ...
                                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Link href={`/${locale}/status-effects/${item.id}`}>
