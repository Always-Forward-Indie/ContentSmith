...
export default function EditNPCPage() {
    ...
    // Reset form with NPC data when it loads
    useEffect(() => {
        if (npc) {
            reset({
                name: npc.name,
    ...
    const onSubmit = (data: EditNpcForm) => {
        ...
        updateNpcMutation.mutate(npcData)
    }
        ...
        <div className="container mx-auto px-4 py-8">
            <div className="flex items-center gap-4 mb-6">
                <Button variant="ghost" size="sm" asChild>
                    <Link href={`/${locale}/npcs/${npcId}`}>
                        <ArrowLeft className="h-4 w-4 mr-2" />
                        {commonT('back')}
                    </Link>
                </Button>
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">{t('edit')} - {npc.name}</h1>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="flex justify-end space-x-4">
                    ...
                    <Button
                        type="submit"
                        disabled={updateNpcMutation.isPending}
                    >
