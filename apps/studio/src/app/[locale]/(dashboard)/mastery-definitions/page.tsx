...
import { Plus, Search, Edit, Trash2, AlertCircle, ChevronLeft, ChevronRight, X } from 'lucide-react'
...
export default function MasteryDefinitionsPage() {
    ...
    const [itemToDelete, setItemToDelete] = useState<MasteryDefinition | null>(null)
    ...
    const { data, isLoading, error, refetch } = trpc.masteryDefinitions.list.useQuery({ search: searchTerm || undefined, page, pageSize: 20 })
    const deleteItem = trpc.masteryDefinitions.delete.useMutation({ onSuccess: () => { refetch(); setItemToDelete(null) } })
    const list: MasteryDefinition[] = data?.data ?? []
        ...
        <TooltipProvider delayDuration={300}>
            <div className="space-y-6">
                <div className="flex items-start justify-between">
                    ...
                    <Link href={`/${locale}/mastery-definitions/create`}>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent">
                                    <TableHead className="pl-4">{t('slug')}</TableHead>
                                    <TableHead>{t('name')}</TableHead>
                                    <TableHead>{t('xpPerPoint')}</TableHead>
                            ...
                            <TableBody>
                                {list.length === 0 ? (
                                    ...
                                    <TableRow key={item.slug} className="group cursor-pointer" onClick={() => router.push(`/${locale}/mastery-definitions/${item.slug}`)}>
                                        <TableCell className="pl-4 font-mono text-sm">{item.slug}</TableCell>
                                        <TableCell className="font-medium">{item.name}</TableCell>
                                        <TableCell>{item.xpPerPoint ?? '—'}</TableCell>
                                        <TableCell>{item.maxPoints ?? '—'}</TableCell>
                                        <TableCell className="pr-4" onClick={e => e.stopPropagation()}>
                                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Link href={`/${locale}/mastery-definitions/${item.slug}`}>
            ...
            <Dialog open={!!itemToDelete} onOpenChange={(open) => !open && setItemToDelete(null)}>
                <DialogContent className="max-w-md">
                    ...
                    <DialogFooter className="gap-2 sm:gap-0">
                        ...
                        <Button variant="destructive" onClick={() => itemToDelete && deleteItem.mutate({ slug: itemToDelete.slug })} disabled={deleteItem.isLoading}>
