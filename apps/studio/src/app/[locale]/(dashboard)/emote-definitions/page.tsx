...
export default function EmoteDefinitionsPage() {
    const t = useTranslations('emoteDefinitions')
    const commonT = useTranslations('common')
    const locale = useLocale()
    const router = useRouter()
    const [itemToDelete, setItemToDelete] = useState<EmoteDefinition | null>(null)

    const { data, isLoading, error, refetch } = trpc.emoteDefinitions.list.useQuery({})
    const deleteItem = trpc.emoteDefinitions.delete.useMutation({ onSuccess: () => { refetch(); setItemToDelete(null) } })
    const list: EmoteDefinition[] = data?.data ?? []
        ...
        <TooltipProvider delayDuration={300}>
            <div className="space-y-6">
                <div className="flex items-start justify-between">
                    ...
                    <Link href={`/${locale}/emote-definitions/create`}>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent">
                                    ...
                                    <TableHead>{t('animationName')}</TableHead>
                            ...
                            <TableBody>
                                {list.length === 0 ? (
                                    ...
                                    <TableRow key={item.id} className="group cursor-pointer" onClick={() => router.push(`/${locale}/emote-definitions/${item.id}`)}>
                                        ...
                                        <TableCell>{item.animationName ?? '—'}</TableCell>
                                            ...
                                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Link href={`/${locale}/emote-definitions/${item.id}`}>
