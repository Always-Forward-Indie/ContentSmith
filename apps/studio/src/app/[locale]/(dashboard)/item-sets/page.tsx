...
import { Plus, Search, Edit, Trash2, Layers, AlertCircle, ChevronLeft, ChevronRight, X } from 'lucide-react'
...
export default function ItemSetsPage() {
    ...
    const [itemToDelete, setItemToDelete] = useState<ItemSet | null>(null)
    ...
    const { data, isLoading, error, refetch } = trpc.itemSets.list.useQuery({ search: searchTerm || undefined, page, pageSize: 20 })
    const deleteItem = trpc.itemSets.delete.useMutation({ onSuccess: () => { refetch(); setItemToDelete(null) } })
    const list: ItemSet[] = data?.data ?? []
        ...
        <TooltipProvider delayDuration={300}>
            <div className="space-y-6">
                <div className="flex items-start justify-between">
                    ...
                    <Link href={`/${locale}/item-sets/create`}>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent">
                                    ...
                                    <TableHead>{t('minPieces')}</TableHead>
                                    <TableHead>{t('maxPieces')}</TableHead>
                            ...
                            <TableBody>
                                {list.length === 0 ? (
                                    ...
                                    <TableRow key={item.id} className="group cursor-pointer" onClick={() => router.push(`/${locale}/item-sets/${item.id}`)}>
                                        ...
                                        <TableCell className="text-sm">{item.minPieces ?? '—'}</TableCell>
                                        <TableCell className="text-sm">{item.maxPieces ?? '—'}</TableCell>
                                        <TableCell className="pr-4" onClick={e => e.stopPropagation()}>
                                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Link href={`/${locale}/item-sets/${item.id}`}>
