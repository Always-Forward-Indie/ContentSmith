...
import { Plus, Search, Edit, Trash2, Zap, AlertCircle, ChevronLeft, ChevronRight, X } from 'lucide-react'
...
export default function ZoneEventsPage() {
    ...
    const [itemToDelete, setItemToDelete] = useState<ZoneEvent | null>(null)
    ...
    const { data, isLoading, error, refetch } = trpc.zoneEvents.list.useQuery({ search: searchTerm || undefined, page, pageSize: 20 })
    const deleteItem = trpc.zoneEvents.delete.useMutation({ onSuccess: () => { refetch(); setItemToDelete(null) } })
    const list: ZoneEvent[] = data?.data ?? []
        ...
        <TooltipProvider delayDuration={300}>
            <div className="space-y-6">
                <div className="flex items-start justify-between">
                    ...
                    <Link href={`/${locale}/zone-events/create`}>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent">
                                    ...
                                    <TableHead>{t('durationSec')}</TableHead>
                                    <TableHead>{t('lootMultiplier')}</TableHead>
                            ...
                            <TableBody>
                                {list.length === 0 ? (
                                    ...
                                    <TableRow key={item.id} className="group cursor-pointer" onClick={() => router.push(`/${locale}/zone-events/${item.id}`)}>
                                        ...
                                        <TableCell className="text-sm">{item.triggerType ?? '—'}</TableCell>
                                        <TableCell className="text-sm">{item.durationSec ?? '—'}</TableCell>
                                        <TableCell className="text-sm">{item.lootMultiplier ?? '—'}</TableCell>
                                        <TableCell className="pr-4" onClick={e => e.stopPropagation()}>
                                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Link href={`/${locale}/zone-events/${item.id}`}>
