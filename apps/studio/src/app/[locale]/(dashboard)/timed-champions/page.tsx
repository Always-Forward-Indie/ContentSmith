...
import { Plus, Trash2, Edit, Trophy, AlertCircle, ChevronLeft, ChevronRight } from 'lucide-react'
...
interface TimedChampion {
    id: number
    slug: string
    zone?: { name?: string; slug?: string } | null
    mobTemplate?: { name?: string; slug?: string } | null
...
export default function TimedChampionsPage() {
    const t = useTranslations('timedChampions')
    const commonT = useTranslations('common')
    const locale = useLocale()
    const router = useRouter()
    const [page, setPage] = useState(1)
    const [itemToDelete, setItemToDelete] = useState<TimedChampion | null>(null)

    const { data, isLoading, error, refetch } = trpc.timedChampions.list.useQuery({ page, pageSize: 20 })
    const deleteItem = trpc.timedChampions.delete.useMutation({ onSuccess: () => { refetch(); setItemToDelete(null) } })
    const list: TimedChampion[] = data?.data ?? []
        ...
        <TooltipProvider delayDuration={300}>
            <div className="space-y-6">
                <div className="flex items-start justify-between">
                    ...
                    <Link href={`/${locale}/timed-champions/create`}>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent">
                                    ...
                                    <TableHead>{t('mob')}</TableHead>
                                    <TableHead>{t('intervalHours')}</TableHead>
                                    <TableHead>{t('windowMinutes')}</TableHead>
                            ...
                            <TableBody>
                                {list.length === 0 ? (
                                    ...
                                    <TableRow key={item.id} className="group cursor-pointer" onClick={() => router.push(`/${locale}/timed-champions/${item.id}`)}>
                                        ...
                                        <TableCell className="text-sm">{item.mobTemplate?.name ?? item.mobTemplate?.slug ?? '—'}</TableCell>
                                        <TableCell className="text-sm">{item.intervalHours ?? '—'}</TableCell>
                                        <TableCell className="text-sm">{item.windowMinutes ?? '—'}</TableCell>
                                        <TableCell className="pr-4" onClick={e => e.stopPropagation()}>
                                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Link href={`/${locale}/timed-champions/${item.id}`}>
